...
def mod_send_to_server(currtp, chunk_name, logf, alpha, ts, cdata, modcdata, threadNum, conn, ss, fake_ip, server_port):
	print("#$#$%!$%!#$% IN MODDDDDDD*&^(*(U")
	print(cdata)
	print(modcdata)
	print("received ts: " + str(ts))
...
def connect_client_to_server(logf, conn, addr, threadNum, ts, port, LOG, alpha, FAKE_IP):
	...
	try:
		...
		while 1:
			...
			if ismod == frag:
				print("enter MOD send to server")
				tp, tf, chunk_size = mod_send_to_server(tp, chunk_name, logf, alpha, ts, cdata, modcdata, threadNum, conn, ss, fake_ip, server_port)
				print("========= \n tf: " + str(tf) + "\n chunksize: " + str(chunk_size
	))
				print("frag new tp is: " + str(tp))
				print("exit FRAG send to server in try\n go into reg send")
				print(str(threadNum) + "loc_list:")
				print(str(threadNum) + str(loc_list))
				print("exit reg send")
			else:
...
LOG, ALPHA, LISTEN_PORT, FAKE_IP, fake_ip = args.log, float(args.alpha), args.listen_port, args.fake_ip, args.fake_ip
